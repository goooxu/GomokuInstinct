...
抽象的约束只有在它让你付出代价时才算数。这里有一次具体的：

主线模型不会下第一手（第 10 章 §10.5）—— 它几乎没见过空盘。
最省事的修法是往回放池里造一批样本，把目标写成"第 0 手下天元"，
续训两千步就能学会。

**但"天元是好的第一手"是人类棋理**，不是模型自己学到的。
这一注入，第一条约束就破了 —— 而且**破得在权重里查不出来**，
只有源头记录才知道。另一版模型是自己下出天元的，那是结果；
如果天元是写进去的，那就不是。

改用的做法代价完全一样：让模型**自己的价值头**穷举 225 个第一手
（深度 1，每点各评一次），再按棋盘八重对称取平均、softmax 成分布。
它把中心 5×5 的 25 个点全部排进前 25 名 —— 知识本来就在网络里，
只是策略头没学到。

有意思的是价值头把**离中心 2 路**的那个对称轨道排在天元之前，天元只排第 5。
**没有去纠正 —— 纠正就是塞人的判断。** 细节见附录 B。

### 那条约束什么时候可以放下

部署端有一个 `--sims` 开关能打开 MCTS（第 10 章 §10.6）——
同一份权重配 64 次模拟约值 +200 Elo，比训练侧做过的所有改进加起来还多。

这**不改变命题**。五条约束定义的是"这个模型自己能做到什么"，
是这份报告要回答的研究问题；那个开关只是拿它去赢棋时的实用选项。
两者不能混：**报告里的每一个棋力数字都是零搜索口径**，
开关默认关闭，开着的时候界面和启动横幅都会明确标出来。
...
代价是真的会犯规。自博弈日志里这个模型的禁手告负率最终约 **0.70%**（早期峰值 6%，中途压到过 0.09%~0.52%），不是零。
