...
两个训练到相近步数的模型摆在一起，同权重测量给出这样的曲线（各档 400 局）：

| MCTS 模拟数 | 模型 F 得分率 | 模型 C 得分率 |
|---|---|---|
| 2 | 49.6% | 51.9% |
| 4 | 44.8% | 46.6% |
| 8 | 41.8% | 33.0% |
...
看起来 F 把搜索"压进权重"压得好得多 —— 64 档还能拿 23.8%，C 只有 11.1%。

**这是个错觉。** 做一次 2×2：让两个零搜索策略分别去打两家的 MCTS。

| 零搜索 ＼ MCTS | C 的 MCTS | F 的 MCTS |
|---|---|---|
| **F** | 38.1 / 32.5 / 21.4 / 13.1 | 41.8 / 34.1 / 29.5 / 23.8 |
| **C** | 33.0 / 21.6 / 17.6 / 11.1 | 39.0 / 28.0 / 27.0 / 20.2 |

（每格四个数依次是 8/16/32/64 档，值为零搜索方的得分率，各 400 局）

**按列读**（固定尺子，比零搜索）：F 在两把尺子下的八个比较全部为正，
但幅度只有 +2 ~ +11pp。

**按行读**（固定零搜索，比 MCTS）：两行都显示 **F 的 MCTS 更弱**，
...
### 为什么 F 的 MCTS 更弱 —— 一个没查明的问题

先说一个**确实成立**的观察。同一批 8,467 个局面上量策略先验：

| | 训练步数 | 采样方式 | 策略熵 | 有效候选数 exp(H) |
|---|---|---|---|---|
| C | 48k | 全局均匀 | 1.392 | **4.0** |
| F | 50k | 尾段重搜 | 0.919 | **2.5** |

F 的先验尖锐得多。**尖锐化多半来自采样方式** —— 尾段重搜产生的目标本来就集中
（残局候选点少，重搜又不加 Dirichlet 噪声），F 的目标熵 0.772 也明显低于 C。
...
**这条解释被实验基本否掉了。** 把 F 的先验用温度软化后再喂给它自己的 MCTS
