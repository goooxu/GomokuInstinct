...
耗时**不再正比于模拟数**，而是正比于**轮数**（= sims / leaves，leaves 钳成
`min(16, sims//8)`）。所以 16、32 那两档没有存在的理由 ——
sims=16 要 112 毫秒而 sims=128 只要 195 毫秒，多花 74% 的时间换 8 倍模拟数，
它们被固定的轮数下限锁住了成本。

> 界面上的耗时标注一度还按旧公式（模拟数 × 11 毫秒）算，400 档写成 4.4 秒
> 而实际 0.44 秒 —— **差一个数量级，会让人照着错的数字选档位。**
> 这类"显示的和实际的不一致"是本项目反复出现的一类问题。

换算成棋力：同样 0.73 秒一手，批量版本（800 次模拟）打原来的版本（64 次模拟）
**143:57，Elo +160**（200 局，6.1 个标准差）。

**1024 以上没有对局证据** —— 验收只测到 800。按曲线趋势应当还在涨，
但收益递减，且批量近似在高模拟数下的表现没测过。
