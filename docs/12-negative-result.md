...
| 指标 | 起点 | 收敛点 |
|---|---|---|
| 超出目标熵（越小越好） | +0.568 | **+0.435** |
| 策略 top1 与 MCTS 一致率 | 62% | **70%** |
| 价值准确率 | 76% | **80.5%** |
| 威胁识别准确率 | — | **98%**（接近饱和） |
...
一个佐证：网络的**威胁识别已经接近满分（98%）**。它清清楚楚地看得见盘上哪里有活三、
...
| **价值头在开局判「白优」** | 与自博弈中黑方胜率更高对不上。已验过符号是对的（造四个胜负已定的局面，四个都判对），是模型的真实判断，值得单独查 |
...
（威胁识别 98%）。一个会慢慢铺双威胁、不靠明显冲四取胜的人类高手，它接不住。

这份报告的价值不在于"做出了一个强引擎"，而在于：

- 把一条完整的自博弈 RL 链路从零搭起来并跑通，每个设计选择的理由都写下来了；
- 把十四个**不报错的失败**记录下来，以及发现它们的方法；
